...
// Building Components
const HomeBuilding: React.FC<{ stage: LifeStage; isSelected: boolean }> = ({ stage, isSelected }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  
  useFrame((state) => {
    if (meshRef.current && isSelected) {
      meshRef.current.position.y = Math.sin(state.clock.elapsedTime * 2) * 0.1 + stage.home.size;
    }
  });

  const getHomeGeometry = () => {
    switch (stage.home.type) {
      case 'studio': return [1, 1, 1];
      case 'apartment': return [1.5, 2, 1.5];
      case 'house': return [2.5, 2.5, 2];
      case 'mansion': return [4, 3, 3];
      default: return [1, 1, 1];
    }
  };

  const [width, height, depth] = getHomeGeometry();

  return (
    <group position={stage.home.position}>
      <Box 
        ref={meshRef}
      ...
      {/* Door */}
      <Box args={[0.3, 0.8, 0.1]} position={[width/2 - 0.1, 0.4, depth/2 + 0.05]}>
...
const WorkplaceBuilding: React.FC<{ stage: LifeStage; isSelected: boolean }> = ({ stage, isSelected }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  ...
  return (
    <group position={stage.workplace.position}>
      <Box 
        ref={meshRef}
        args={[2, stage.workplace.height, 2]}
        position={[0, stage.workplace.height/2, 0]}
      >
        <meshStandardMaterial 
          color={stage.workplace.color}
          opacity={isSelected ? 1 : 0.7}
          transparent={!isSelected}
        />
      </Box>
      {/* Windows */}
      {Array.from({ length: Math.floor(stage.workplace.height) }, (_, i) => (
        <Box 
          key={i}
          args={[0.3, 0.3, 0.05]} 
          position={[0.8, 0.5 + i * 0.8, 1.01]}
        >
      ...
      {isSelected && (
        <Text
          position={[0, stage.workplace.height + 0.5, 0]}
          fontSize={0.25}
          color="white"
          anchorX="center"
          anchorY="middle"
...
const VehicleModel: React.FC<{ stage: LifeStage; isSelected: boolean }> = ({ stage, isSelected }) => {
  if (stage.vehicle.type === 'none') return null;

  const getVehicleSize = () => {
    switch (stage.vehicle.type) {
      case 'bike': return [0.3, 0.2, 0.8];
      case 'car': return [0.8, 0.4, 1.5];
      case 'luxury': return [1, 0.5, 2];
      case 'electric': return [0.9, 0.4, 1.8];
      default: return [0.8, 0.4, 1.5];
    }
  };

  const [width, height, length] = getVehicleSize();

  return (
    <group position={stage.vehicle.position}>
      <Box args={[width, height, length]} position={[0, height/2, 0]}>
        <meshStandardMaterial color={stage.vehicle.color} />
      </Box>
      {/* Wheels */}
      <Cylinder args={[0.15, 0.15, 0.1]} position={[width/2 - 0.1, 0.15, length/2 - 0.2]} rotation={[0, 0, Math.PI/2]}>
...
const LifestyleElements: React.FC<{ stage: LifeStage; isSelected: boolean }> = ({ stage, isSelected }) => {
  return (
    <group>
      {/* Gym */}
      {stage.lifestyle.gym && (
        <Box args={[1.5, 1, 1]} position={[6, 0.5, 2]}>
          <meshStandardMaterial color="#FF6B6B" />
        </Box>
      )}
      
      {/* Restaurants */}
      {Array.from({ length: stage.lifestyle.restaurants }, (_, i) => (
        <Cylinder 
          key={i}
          args={[0.5, 0.5, 1]} 
          position={[8 + i * 1.2, 0.5, -2]}
        >
      ...
      {/* Family indicators */}
      {stage.lifestyle.family && (
        <group position={[stage.home.position[0] + 2, 0, stage.home.position[2]]}>
          <Sphere args={[0.2]} position={[0, 0.2, 0]}>
            <meshStandardMaterial color="#FFB6C1" />
          </Sphere>
          <Sphere args={[0.15]} position={[0.4, 0.15, 0]}>
            <meshStandardMaterial color="#FFB6C1" />
          </Sphere>
        </group>
      )}
      ...
      {/* Travel indicator */}
      {stage.lifestyle.travel && (
        <Sphere args={[0.3]} position={[10, 2, 0]}>
